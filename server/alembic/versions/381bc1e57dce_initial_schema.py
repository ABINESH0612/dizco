...
def upgrade() -> None:
    ...
    if "addresses" not in existing_tables:
        op.create_table(
            ...
            sa.Column("is_default", sa.Boolean(), nullable=True, server_default=sa.text("1")),
            sa.PrimaryKeyConstraint("id"),
        )
    ...
    if "reviews" not in existing_tables:
        op.create_table(
            ...
            sa.Column("status", sa.String(length=50), nullable=False, server_default="pending"),
            sa.Column("is_verified_purchase", sa.Boolean(), nullable=False, server_default=sa.text("1")),
            sa.Column("created_at", sa.DateTime(), nullable=True),
            sa.Column("updated_at", sa.DateTime(), nullable=True),
            sa.PrimaryKeyConstraint("id"),
        )
